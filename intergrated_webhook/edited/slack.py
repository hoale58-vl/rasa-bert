...
class SlackBot(SlackClient, OutputChannel):
    ...
    def __init__(self, token: Text, slack_channel: Optional[Text] = None, send_channel: Optional[Text] = None) -> None:
        super(SlackBot, self).__init__(token)
        if send_channel is not None:
            channel_info=super(SlackBot, self).api_call("channels.info",channel=send_channel)
            if "channel" in channel_info:
                channel_name = channel_info["channel"]["name"]
                if channel_name in slack_channel.split(' '):
        ...
        **kwargs: Any
        ...
        if self.slack_channel:
            ...
            if len(buttons) > 5:
                ...
                return await self.send_text_message(self.slack_channel, text, **kwargs)
    ...
    async def send_custom_json(
        ...
        if self.slack_channel:
            json_message.setdefault("channel", self.slack_channel)
...
class SlackInput(InputChannel):
    ...
    async def process_message(self, request: Request, on_new_message, text, sender_id, send_channel=None):
        ...
        try:
            out_channel = SlackBot(self.slack_token, self.slack_channel, send_channel)
            ...
            if request.form:
                ...
                if "challenge" in output:
                    ...
                    return await self.process_message(
                        ...
                        send_channel=output["event"]["channel"]
                    )
